...
DTHD_states <- DTHD_states %>% 
  group_by(Region) %>% 
  mutate(RegionMean = mean(DeathRate, na.rm = TRUE))
...
ggplot(data = DTHD_states,
       mapping = aes(x = long, y = lat, group = group, fill = RegionMean)) +
  geom_polygon(color = "gray30", size = 0.1) +
  coord_map(projection = "albers", lat0 = 39, lat1 = 45) +
  scale_fill_gradient(low = "#E2B8B2", high = "#CB454A") +
...
ggplot(data = DTHD_states,
       mapping = aes(x = long, y = lat, group = group, fill = DivisionMean)) +
  geom_polygon(color = "gray30", size = 0.1) +
  coord_map(projection = "albers", lat0 = 39, lat1 = 45) +
  scale_fill_gradient(low = "#E2B8B2", high = "#CB454A") +
  labs(title = "Regional Death Rates") + 
...
dt_m <- train(DeathRate ~ Division + Year, data = DTHD, tuneLength = 19L,
              method = 'rpart2')
...
State, Division and Region but Deep Learning models don't care.  We include 
these additional features as surrounding geos may provide predictive value for a 
given state.  In future analysis, we could explore dropping out Region and/or 
Division.
...
Note that we did a grid search of hyperparameters to optimize the model.  Also, R-Squared is not normally a metric we calcualte with models.  Usually we score models with Mean Absolute Error (MAE), RMSE, or even log versions of these.  We added a function to caluclate R-Squared for the model so we could directly compare performance with previous models in this project.
...
#      FLAGS <- list('neuron1'=neuron1, 'neuron2'=neuron2, 'lr'=lr)
      ...
      FLAGS <- list('neuron1' = 256, 'neuron2' = 32, 'lr' = 0.001)
...
result of hypertuning led to a model with one hidden layers with 256 hidden 
nodes and a second hidden layer with 32 hidden nodes.  We found a learning 
rate of 0.001 to be optimal.  Note that with more time, we could have explored 
additinal hidden layers, dropout layers, etc. to further refine refine.  
Our R-squared on the training data was **`r scores[[1]]$trn_r2`** and on 
the holdout test data, **`r scores[[1]]$test_r2`**.  When training models, 
there is variability.  In general we expect the training scores to be higher 
as a model memorizes and overfits to the training data.  In this case, 
the hold out was very close and slightly higher suggesting we did not 
overfit and the model is still generalizing.  Since our training data 
was 2000-2016 and our holdout test data was 2017, we suspect the model 
started picking up on trends over time and was able to predict the following
year with better accuracy.  We could confirm this by retraining with 2000-2015
and seeing how well it predict 2016.  We would suspect that as we drop back 
one year at a time, the model will capture less of the timeseries aspect and
perform worse.  Since this was timeseries data we did *NOT* shuffle the data ...
we wanted the model to pick up on trends within the data.
